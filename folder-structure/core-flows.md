...
1. Maintainer moves the existing Svelte app into the new repository and commits.
1. Maintainer adds `gsnake-web` as a git submodule in the parent repo.
1. Maintainer updates root references, scripts, and docs to point to the `gsnake-web` submodule path.
1. Maintainer confirms all Svelte-related configs now live under `gsnake-web`, leaving Playwright e2e tests in the parent repo.
...
1. Developer runs `gsnake-core` native tests (Cargo).
1. Developer builds bindings for target platforms as needed (WASM via `wasm-pack build`, Python via local editable install, CLI via Cargo).
1. Developer verifies changes using the internal CLI tool (when CLI is affected).
1. Developer commits changes to the `gsnake-core` repository.
...
1. Web developer ensures `package.json` uses a `file:` dependency pointing to `../gsnake-core/engine/bindings/wasm/pkg`.
1. Developer runs the root script that rebuilds the WASM package before dev/test.
1. Developer runs `npm install` if necessary.
1. Developer runs the Svelte dev server to verify integration.
1. UI-specific changes are committed to the `gsnake-web` repository.
...
1. Developer updates the requirement in `pyproject.toml` to the next SemVer target.
1. Developer runs the Python application/tests to verify integration.
1. Python-specific changes are committed to the appropriate Python repository when it exists.
...
1. Maintainer updates submodule pointers in the parent repository once individual modules are ready.
1. Global CI runs integration builds/tests (including Playwright) using Python orchestration and is gated on pinned submodule SHAs, then deploys the Svelte web app.
