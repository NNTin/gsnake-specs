...
1. Playwright starts in headless mode (no visible browser UI)
1. Local dev server spins up (or connects to existing one)
1. Tests execute in parallel workers
1. Terminal displays live progress (dots/ticks)
1. **If Success:** Terminal shows green "Passed" summary with duration
1. **If Failure:** Terminal shows red "Failed" summary with specific error details and diffs
...
1. Dashboard displays overview of test suite status
1. Developer navigates to "Behaviors" or "Suites" tab
1. Developer selects the failed test case
1. Report displays:
...
1. Developer plays video to observe the exact moment of failure (e.g., snake didn't fall when expected)
...
1. **Reaction - Gravity Application:**
