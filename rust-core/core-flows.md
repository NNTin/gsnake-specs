...
2.  **Level Injection:** The host (TypeScript) injects the current level data into the Rust core.
3.  **Interactive Loop:**
...
4.  **Exit:** User presses **Q** to terminate the process and return to the shell.
...
2.  **Auto-Advance:** In the CLI, the next level loads automatically after a short delay or keypress. In Web, the user interacts with a modal to proceed.
3.  **Data Consistency:** Both platforms utilize the same `levels.json` structure, ensuring the puzzles and mechanics remain identical regardless of the interface.
...
2.  **Detection:** CLI checks terminal dimensions against Level 1 grid size.
3.  **Feedback:** CLI displays a "Terminal too small" warning and waits for resize or exit.

**Flow: Environmental Error (Shared)**
1.  **Trigger:** Missing `levels.json` or malformed data injection.
2.  **Action:** The engine/host throws an error.
3.  **Feedback:** The UI/CLI displays the error message and a traceback for debugging, then terminates or halts.
