...
2) Initialize the wasm module (`init()`), then fetch levels via `getLevels()`.
3) Construct `WasmGameEngine` with a `LevelDefinition` and register `onFrame` callback.
4) Render strictly from `Frame.grid` and `Frame.state` as provided; do not derive gameplay state.
5) Send user input via `processMove(direction)` using `Direction` enum values.
Feedback & State:
...
2) Regenerate TS types and confirm all enums serialize as strings matching ts-rs output.
3) Remove or update any UI code that normalizes or maps payloads.
4) Add/adjust integration tests to validate serialized payloads and error shapes.
Feedback & State:
...
2) If initialization fails, retry once.
3) If it fails again, surface a fatal error state and log the typed error payload.
Feedback & State:
...
2) Fail the build or integration tests on any enum/string mismatch or missing field.
3) Surface a clear error message that identifies the mismatched type.
Feedback & State:
